...
DEPLOYMENT_NAME=$(kubectl get pods --namespace ${CLUSTER_NAMESPACE} -o json | jq -r '[ .items[] | select(.spec.containers[]?.image | contains("'"${IMAGE_REPOSITORY}"'")) | .metadata.labels.app] [1]' )
